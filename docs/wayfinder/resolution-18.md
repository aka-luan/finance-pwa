...
O domínio **só é tocado na confirmação** (transação atômica). Até lá o app não entra no Termômetro. “Wizard incompleto” **não** é um estado no Postgres — é UI/sessão — justamente para não deixar gravações parciais.
...
| **Wizard incompleto** | Wizard aberto; progresso só em memória. **Nenhuma** linha de `account_anchor` / `monthly_budget` / `daily_estimate` / recorrências do confirm | Continua no wizard; refresh → wizard do zero |
| **Planejamento concluído** | `needsFirstRun` = falso (após confirm atômico do wizard **ou** backup que já deixa o app pronto) | Termômetro (`renderHoje`) |
| **Backup restaurado** | Não é flag: `importBackup` substitui as tabelas conhecidas; em seguida reaplica-se o mesmo `needsFirstRun` | Termômetro se pronto; senão wizard |
...
- Backups v1 / instalações atuais podem ter saldo + estimativa sem composição (#19). Forçar wizard esconderia um Termômetro já honesto.
- Composição ausente com estimativa presente → Termômetro ok; o **Planejamento** cria a primeira `monthly_budget`.
- Confirm **novo** (pós-#17) sempre grava os três; o gate mínimo evita mentir no hero/saldo.

Ausência **só** de saldo ou **só** de estimativa → ainda `needsFirstRun` (não dá para montar o Termômetro sem os dois).
...
Recalibrar é a superfície **Planejamento** (ADR 0006), só quando
`NOT needsFirstRun`. O caminho `Configurações → Recalibrar
planejamento` não existe: Configurações tem um atalho “Abrir
planejamento”.

## Conclusão (confirm) — atômica

Uma única `db.transaction` no confirm do primeiro uso:
...
No primeiro uso, ao sucesso → vai para o Termômetro (`renderHoje`).
Recalibração contínua não passa por este confirm: o Planejamento
autosave via `savePlanningAssumptions`.

## Retomada

- **Dentro da sessão:** o stepper (#15) guarda o passo em memória; Voltar/Continuar não persiste domínio.
- **Opcional de UX (não feito):** espelhar o draft em `sessionStorage`
  para sobreviver a reload da aba. Não há draft de sessão no código.
- **Não** há retomada de wizard incompleto via Postgres: matar o app a meio caminho deixa o banco ainda “não pronto” e o próximo open reabre o wizard do início.
...
2. Após restore, recalcular `needsFirstRun`:
   - pronto → Termômetro (sai do wizard);
...
#19: ao acrescentar `monthly_budget*`, backups v1 sem essas tabelas precisam continuar restauráveis e cair no Termômetro quando já tiverem âncora + estimativa — alinhado a este gate.
